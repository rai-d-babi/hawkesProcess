...
# define non-vectorised conditional intensity fucntion of univariate HP at time t
def intensity_func(t_seq, event_times, baseline, alpha, beta, node=0):
    """
    t_seq - list/array containing series of time points up to time t, i.e. [0,t] or a list containing a single moment of time t, i.e., [t]
    event_times: list/array containing time stamps
    node: node of multivariate process
    baseline - baseline intensity of the Hawkes process
    alpha -  excitation parameter of HP
    beta - decay parameter of HP
    """
    # t - current time
    # event_times contains all event times T_i < t 
    rate = baseline # intensity
    num_of_events = np.shape(event_times)[0] # total number of events observed between [0,T]
    ...
    while (t_seq > event_times[i]):
        rate = rate + exp_kernel(alpha, beta, t_seq-event_times[i])
        i += 1 # next event
        if (i >= num_of_events):
            break
    #print(t-T_i)
...
# Define vectorised univariate conditional intensity function at time t
def univ_cif(t, timestamp, mu, alpha, beta):
    """ Conditional intensity function of a univariate Hawkes process with exponential triggering kernel.       "
    " Parameters:                                                                                  "
    " - mu corresponds to the background intensity of the HP.                                        "
    " - alpha corresponds to the jump intensity that indicates the jump in intensity upon arrival of an event.  "
    " - beta is the decay parameter that governs the exponential decay of intensity.                 """
...
def multiv_cif(t, timestamp, mu, alpha, beta):
    """ Conditional intensity function of a (M-variate) multivariate Hawkes process with exponential triggering kernel.       "
    ...
    for M in range(num_of_nodes):
        ...
        for n in range(num_of_nodes):
             # masks/select out all index of timestamps across type-n event such that t precedes moments of time in timstamp array
             # i.e., mask_index contains all i such that t_i < t
            mask_index = np.where(timestamp[n] < t)
            lambdas[M] += np.sum(alpha[M][n]*np.exp(-beta[M][n]*np.subtract(t, timestamp[n][mask_index]))) # contribution of node n
    return lambdas



## Generate N number of timestamps of univariate HP
def simulate_univ_ts(mu, alpha, beta, num_of_tstamp = 100, seed=None, output_rejected_data=False):
    """
    Inputs:
    ...
    # Initialisation
    #################
    #rng = default_rng(seed) # Call default_rng to get a new instance of a Generator, then call its methods to obtain samples from standard uniform distribution. 
    ...
    rejected_points = []; rpy = [] # containter for rejected time points and their correspodning intensities
    M_x = []; M_y = [] # M_y stores Maximum or upper bound of current times while M_x stores their x-values
    ...
    while(i < num_of_tstamp):
        ...
        lambda_star = intensity_func(t+epsilon, ts, mu, alpha, beta) # compute upper bound of intensity using conditional intensity function
        ...
        lambda_t = intensity_func(t, ts, mu, alpha, beta) # compute intensity function at current time t
        ##########################
...
def simulate_timestamps_till_horizon(mu, alpha, beta, Thorizon = 60, seed=None, node=None, output_rejected_data=False):
    """
    Inputs:
    mu, alpha, beta are parameters of intensity function of HP
    """
...
def simulate_multivariate_ts(mu, alpha, beta, num_of_nodes=-1, Thorizon = 60, seed=None, output_rejected_data=False):
    """
    Inputs: 
